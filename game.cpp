...
bool Game::init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow("Mariana Trench Fishing", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (gWindow == NULL)
		{
...
void Game::collision(Hook *hook, Boat *boat)
{
	...
	while (p != NULL)
	{
		...
		if (DC->getDirection() == false)
		{
			...
			{
				Points += *hook * DC; //Operator Overload
		...
		while (K != NULL)
		{
			...
			{
				version = *hook + DC3; //Operator Overload
				if (version == 3)
				{
...
void Game::close()
{
	...
	SDL_DestroyTexture(Misce);
	Misce = NULL;
	SDL_DestroyTexture(Score);
	Score = NULL;
	SDL_DestroyTexture(fire);
	fire = NULL;
	SDL_DestroyTexture(Drogon);
	Drogon = NULL;
	SDL_DestroyTexture(Booster);
	Booster = NULL;
	//Destroy window
